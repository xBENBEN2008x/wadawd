...
toggleLabel.Text = "GUI Toggle: Right-Shift"
toggleLabel.Size = UDim2.new(0, 300, 0, 40)
toggleLabel.Position = UDim2.new(1, -320, 0, 20)
...
contentLabel.Text = "-- Nexor Release GUI 1.11.2025.\n-- Adding Misc Soon!\nAdding Key System: 10.11.2025"
...
--// Content Rows
local function createContentRow(data)
	...
	runStatus.Parent = row
	runStatus.Size = UDim2.new(0, 120, 1, 0)
	runStatus.Position = UDim2.new(1, -220, 0, 0)
	runStatus.BackgroundTransparency = 1
	runStatus.Font = Enum.Font.GothamBold
	runStatus.TextSize = 18
	runStatus.TextColor3 = Color3.fromRGB(255, 255, 255)
	runStatus.Text = ""

	local runButton = Instance.new("TextButton")
	runButton.Parent = row
	runButton.Size = UDim2.new(0, 80, 0, 35)
	runButton.Position = UDim2.new(1, -90, 0.5, -17)
	...
	runButton.MouseButton1Click:Connect(function()
		runStatus.Text = "Loading..."
		runStatus.TextColor3 = Color3.fromRGB(255, 255, 0)
		task.spawn(function()
			local success, err = pcall(data.Run)
			if success then
				runStatus.Text = "Done!"
				runStatus.TextColor3 = Color3.fromRGB(0, 255, 0)
			else
				runStatus.Text = "Error!"
				runStatus.TextColor3 = Color3.fromRGB(255, 0, 0)
				warn(err)
			end
		end)
	end)
...
local contentButton = createButton("Content")
local miscButton = createButton("Misc")
local creditsButton = createButton("Credits") -- Neue Kategorie
...
miscButton.MouseButton1Click:Connect(function()
	contentLabel.Text = "-- Misc Coming Soon!\n-- GUI Config & more"
	contentLabel.Visible = true
	scrollFrame.Visible = false
end)
